<div align="center">

<h1><code>Leonardo Vanni</code></h1>

<img src="./typing-header.svg" alt="Backend Software Engineer" />

<p><em>Construindo sistemas que geram impacto no mundo real.</em></p>

<a href="https://portfolio-v2-omega-swart.vercel.app/">Portfólio</a>
&nbsp;·&nbsp;
<a href="https://www.linkedin.com/in/leonardo-vanni-bonavigo-6a387020b/">LinkedIn</a>
&nbsp;·&nbsp;
<a href="https://instagram.com/leonardobonavigo">Instagram</a>

<br><br>

<h3><code>leo@github ~ $ trace request --flow</code></h3>
<img src="./architecture.svg" width="860" alt="Backend request flow: Client → API → Cache → Database" />

<br><br>

<h3><code>leo@github ~ $ ./contributions.sh</code></h3>
<img src="./contrib-heatmap.svg" width="860" alt="Contribution heatmap" />

<br><br>

<h3><code>leo@github ~ $ whoami</code></h3>
<table>
  <tr>
    <td valign="top"><img src="./leo-ascii.svg" width="370" alt="ASCII portrait" /></td>
    <td valign="top"><img src="./info-card.svg" width="490" alt="Info card" /></td>
  </tr>
</table>

<br><br>

<h3><code>leo@github ~ $ ./stats.sh</code></h3>
<table>
  <tr>
    <td valign="top"><img src="./stats-card.svg" width="440" alt="GitHub stats" /></td>
    <td valign="top"><img src="./langs-card.svg" width="440" alt="Top languages" /></td>
  </tr>
</table>

<img src="https://streak-stats.demolab.com/?user=LeonardoVanni21&hide_border=true&background=0d1117&stroke=21262d&ring=39d353&fire=39d353&currStreakLabel=39d353&sideLabels=c9d1d9&dates=8b949e&currStreakNum=c9d1d9&sideNums=c9d1d9" alt="GitHub streak" />

</div>
